...
class gui:
    ...
    def get_probablities(self):
        ...
        #counting total no of experiments
        total_experiments=0
        for i in self.occurences:
            total_experiments=total_experiments+i
        
        #proballity of each event
        self.probablity=[round(i/total_experiments,3) for i in self.occurences]
        
        #mean
        self.mean=0
        for i in range(len(self.events)):
            self.mean=self.mean+self.events[i]*self.probablity[i]
        
        #population variance
        self.population_variance=0
        for i in range(len(self.events)):
            self.population_variance=self.population_variance+(self.mean-self.events[i])**2
        self.population_variance=self.population_variance/len(self.events)
        
        
    def plot_sample(self):
        #occurence of events in 1000 total observations
        thousand_exp_results=[i*1000 for i in self.probablity]
        #getting sample size
        sample_size=int(self.sample_size_text.get())
        #no of times sample is needed
        experiment_size=int(self. no_of_experiments_text.get())
        #new graph window
        new_plot=plt.figure(2)
        #observationas in sampling distribution
        ...
        sample_mean_occurence_keys=np.array(list(sample_mean_occurence.keys()))
        sample_mean_occurence_values=np.array(list(sample_mean_occurence.values()))
        sample_distribution_middle=sample_mean_occurence_keys*sample_mean_occurence_values
        sample_distribution_middle=np.sum(sample_distribution_middle)/experiment_size
        maximum_occurence=max(sample_mean_occurence_values)
        standard_error=self.population_variance/sample_size
        
        plt.bar(sample_mean_occurence_keys,sample_mean_occurence_values,width=1/sample_size)         
        plt.plot((sample_distribution_middle,sample_distribution_middle),(0,maximum_occurence),color="Red")

        plt.plot((sample_distribution_middle-standard_error,sample_distribution_middle-standard_error),(0,maximum_occurence),color="green")
        plt.plot((sample_distribution_middle+standard_error,sample_distribution_middle+standard_error),(0,maximum_occurence),color="green")
        
        plt.plot((sample_distribution_middle-2*standard_error,sample_distribution_middle-2*standard_error),(0,maximum_occurence),color="brown")
        plt.plot((sample_distribution_middle+2*standard_error,sample_distribution_middle+2*standard_error),(0,maximum_occurence),color="brown")
        
        plt.plot((sample_distribution_middle-3*standard_error,sample_distribution_middle-3*standard_error),(0,maximum_occurence),color="pink")
        plt.plot((sample_distribution_middle+3*standard_error,sample_distribution_middle+3*standard_error),(0,maximum_occurence),color="pink")
        new_plot.show()                 
